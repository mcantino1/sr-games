...
selectDiv = document.getElementById("gameSelect");
startDiv = document.getElementById("menuStart");
...
function swapSettingElements(from, to){
	myEls = from.children;
	while (myEls.length > 0){
		to.appendChild(myEls[0]);	 
...
function menuChange(btn){
	document.getElementById(btn.getAttribute("myScreen")).classList.add("hideMe")
	document.getElementById(btn.getAttribute("tarScreen")).classList.remove("hideMe")
}
...
function revealMap(){
	...
	let pos = toPos(r, c);
	let nCell = document.getElementById("cell_" + pos);
	if(nCell){
		nCell.classList.remove("unknown");
		nCell.classList.add("visited");
	}
...
function renderMap() {
  ...
  for (let r = 0; r < level.rows; r++) {
	  ...
	  myRow.setAttribute("class", "mapRow");
	  mapEl.appendChild(myRow);
    for (let c = 0; c < level.cols; c++) {
      const pos = toPos(r, c);
...
function activateCell(pos){
	...
	if (targetLevel == currentLevelId){
		...
		else{		announce("I've got nothing else to say.");}
		updateUI();
	}
