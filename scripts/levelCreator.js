...
function updateIconBank(){
	document.getElementById("iconName").value = iconSelect.value;
	vectorAttributes = [document.getElementById("iconRows"), document.getElementById("iconCols")]
	for (att of vectorAttributes){
		myMeta = att.getAttribute("meta")
...
function refreshCells(){
	...
	for (var i = 0; i < allCells.length; i++) {
		...
		if (found) {
			...
			if (itemIcons[icon]) {
				// Display SVG icon
				cell.innerHTML = itemIcons[icon];
				iconRef = cell.firstElementChild;
				if (iconRef.getAttribute("rowspan") || iconRef.getAttribute("colspan")){
...
function updateCellSizing(cell){
	...
	//mark overlap cell
	//data-row="2"
	homeRow = cell.getAttribute("data-row");
	homeCol	= cell.getAttribute("data-col");
	homePos = cell.getAttribute("data-pos");
	...
	for(let r = 0; r < iconrowspan; r++){
		...
		console.log(thisPos);
		if(thisPos != homePos && document.getElementById('cell_' + thisPos)){
			
			
			thisCell = document.getElementById('cell_' + thisPos);
			console.log(thisCell);
			thisCell.setAttribute("overlapped", homePos)
...
function announceCell(pos) {
	var found = null;
	found = items[getItemIndex(pos)];

	var cell = document.getElementById('cell_' + pos);
	
	var announcement = 'Cell ' + pos;
	
	if(cell.getAttribute("overlapped")){
		overPos = cell.getAttribute("overlapped")
		//function getItemIndex(pos){

		overItem = items[getItemIndex(overPos)];
		overName = overItem.type;
		if (overItem.meta && overItem.meta.name){
			overName = overItem.meta.name;
		}
		announcement += ". Overlapped by " + overName + " in " + overPos;
